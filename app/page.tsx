...
export default function Home() {
  ...
  const css = `
    ...
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box }
    ...
    .nav.solid .nav-logo-name { color:#071120 }
    ...
    .nav.solid .burger span { background:#071120 }
    ...
    .mobile-menu-dark { background:rgba(0,18,51,.97); backdrop-filter:blur(24px) }
    ...
    .calc-left { background:rgba(0,18,51,.45); border:1px solid rgba(201,168,76,.08); padding:40px }
    .calc-right { background:rgba(0,18,51,.45); border:1px solid rgba(201,168,76,.08); padding:40px; display:flex; flex-direction:column; gap:2px }
    ...
    .about-item-t { font-size:8px; letter-spacing:2.5px; text-transform:uppercase; color:#071120; margin-bottom:4px; font-weight:500 }
    ...
    .stat-box { background:#071120; padding:28px 24px }
      ...
      <div className={`mobile-menu ${menuOpen ? 'open' : ''} ${navSolid ? 'mobile-menu-solid' : 'mobile-menu-dark'}`}>
        {[
          ...
          <a key={href as string} href={href as string}
            ...
            style={{ color: navSolid ? '#071120' : 'rgba(255,255,255,.85)', borderBottom: '1px solid rgba(201,168,76,.08)' }}>
            {label}
          </a>
        ))}
      </div>
        ...
        <div className="calc-wrap">
          <div className="calc-left">
            ...
            <select className="c-sel" value={calcKarat} onChange={e => setCalcKarat(parseFloat(e.target.value))}>
              <option value={0.9166} style={{ background: '#071120' }}>22 {T('Ayar', 'Karat')} — 916‰</option>
              <option value={0.75} style={{ background: '#071120' }}>18 {T('Ayar', 'Karat')} — 750‰</option>
              <option value={0.585} style={{ background: '#071120' }}>14 {T('Ayar', 'Karat')} — 585‰</option>
              <option value={0.375} style={{ background: '#071120' }}>9 {T('Ayar', 'Karat')} — 375‰</option>
            </select>
            ...
            <select className="c-sel" value={calcKdv} onChange={e => setCalcKdv(parseFloat(e.target.value))}>
              <option value={0} style={{ background: '#071120' }}>{T('KDV Dahil Değil', 'Excl. VAT')}</option>
              <option value={0.18} style={{ background: '#071120' }}>%18 KDV</option>
              <option value={0.20} style={{ background: '#071120' }}>%20 KDV</option>
            </select>
          </div>
